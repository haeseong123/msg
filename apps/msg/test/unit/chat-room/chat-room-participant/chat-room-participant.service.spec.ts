...
describe('ChatRoomController', () => {
    ...
    beforeEach(async () => {
        ...
        chatRoomParticipantService = module.get<ChatRoomParticipantService>(ChatRoomParticipantService);
    ...
    describe('특정 채팅방에 유저를 한 명 초대합니다.', () => {
        it('성공', async () => {
            ...
            const saveDto = new ChatRoomParticipantSaveDto(chatRoomId, inviterUserId, inviteeUserId);
            ...
            jest.spyOn(inviter, 'validateTargetIdsAllFollowing').mockImplementation(([]) => { });
            jest.spyOn(chatRoom, 'findParticipantByUserIdOrThrow').mockImplementationOnce((_) => ChatRoomParticipant.of(1, 1));
            jest.spyOn(chatRoomService, 'validateChatRoomCapacityForParticipants').mockImplementation((_) => { });
            jest.spyOn(chatRoom, 'participate').mockImplementation((_) => { });
            jest.spyOn(chatRoomService, 'saveByEntity').mockResolvedValue(chatRoom);
            jest.spyOn(chatRoom, 'findParticipantByUserIdOrThrow').mockImplementationOnce((_) => saved);
    ...
    describe('채팅방 나가기', () => {
        ...
        beforeEach(() => {
            ...
            removeDto = new ChatRoomParticipantRemoveDto(participantId, userId, chatRoomId);
            chatRoom = ChatRoom.of('', []);
            participant = ChatRoomParticipant.of(chatRoomId, userId);

            jest.spyOn(chatRoomService, 'findByIdOrThrow').mockResolvedValueOnce(chatRoom);
            jest.spyOn(chatRoom, 'findParticipantByUserIdOrThrow').mockReturnValueOnce(participant);
        ...
        it('성공: 채팅방 나가기', async () => {
            ...
            const leaveChatRoomSpy = jest.spyOn(chatRoom, 'leaveChatRoom').mockImplementation(() => { });
