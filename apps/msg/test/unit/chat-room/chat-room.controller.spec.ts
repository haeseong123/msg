...
describe('ChatRoomController', () => {
  ...
  describe('채팅방 전부 가져오기', () => {
    it('성공', async () => {
      ...
      jest
        .spyOn(chatRoomService, 'findAllByUserId')
  ...
  describe('채팅방 상세 가져오기', () => {
    it('성공', async () => {
      // Given
      const chatRoomWithMessagesDto = new ChatRoomWithMessagesDto(
        1,
        '',
        [],
        [],
      ...
      jest
        .spyOn(chatRoomService, 'findChatRoomWithMessages')
        .mockResolvedValue(chatRoomWithMessagesDto);
