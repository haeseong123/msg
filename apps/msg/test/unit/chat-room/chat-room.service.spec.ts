...
describe('ChatRoomService', () => {
    ...
    describe('회원이 참여중인 특정(하나의) 채팅방을 가져옵니다.', () => {
        it('성공', async () => {
            ...
            jest.spyOn(messageService, 'findAllByChatRoomId').mockResolvedValue(messages);
            jest.spyOn(chatRoom, 'findParticipantByUserIdOrThrow').mockImplementation((userId: number) => ChatRoomParticipant.of(chatRoom.id, userId));
    ...
    describe('채팅방을 생성합니다.', () => {
        it('성공', async () => {
            ...
            jest.spyOn(chatRoomService, 'validateChatRoomCapacityForParticipants').mockImplementation(() => { });
            jest.spyOn(userService, 'findByIdOrThrow').mockResolvedValue(host);
            jest.spyOn(host, 'validateTargetIdsAllFollowing').mockImplementation((_set) => { });
        ...
        it('실패: 초대 목록에 hostId가 존재', async () => {
            // Given
            const hostId = 1;
            const chatRoomSaveDto = new ChatRoomSaveDto(hostId, 'title', [hostId, 2, 3, 4, 5]);
    ...
    describe('채팅방이 participantSize 만큼의 인원을 수용할 수 있는지 확인합니다.', () => {
        it('성공', () => {
            ...
            const spy = jest.spyOn(chatRoomService, 'validateChatRoomCapacityForParticipants')
        ...
        it('실패: 최대 수용 인원을 초과함', () => {
            ...
            // When
            const fail = () => chatRoomService.validateChatRoomCapacityForParticipants(participantSize);
