...
export class ChatRoomParticipantService {
  ...
  async save(dto: ChatRoomParticipantSaveDto): Promise<ChatRoomParticipant> {
     ...
     */
    this.chatRoomService.validateChatRoomCapacityForParticipants(
      chatRoom.getParticipantsSize() + 1,
    );
     ...
     */
    const savedParticipant = updatedChatRoom.findParticipantByUserIdOrThrow(
      dto.inviteeUserId,
    );
