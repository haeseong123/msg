...
export class ChatRoomSaveDto {
  ...
  constructor(hostUserId: number, title: string, invitedUserIds: number[]) {
  ...
  toEntity(): ChatRoom {
    ...
    ].map((userId) => ChatRoomParticipant.of(null, userId));

    return ChatRoom.of(this._title, chatRoomParticipants);
  }
