...
export class ChatRoom extends AssignedIdAndTimestampBaseEntity {
  ...
  @OneToMany(
    () => ChatRoomParticipant,
    (chatRoomParticipant) => chatRoomParticipant.chatRoom,
    {
      /** 저장/갱신될 때 같이 저장 */
      cascade: true,
    },
  ...
  static of(title: string, participants: ChatRoomParticipant[]): ChatRoom {
  ...
  participate(participant: ChatRoomParticipant) {
    const existingParticipant = this.findparticipantByUserId(
      participant.userId,
    );
